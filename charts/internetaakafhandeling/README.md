...
| nameOverride | string | `""` |  |
| nieuweInternetaakNotificatie.notification.pollerMessage | string | `"Poller uitgevoerd om:"` | Message written to the logs when a poller run starts |
| nieuweInternetaakNotificatie.schedule | string | `"*/15 * * * *"` | Cron expression controlling how often the poller checks for new internetaken |
| nodeSelector | object | `{}` |  |
...
| tolerations | list | `[]` |  |
| verlopenContactverzoekHerinneringNotificatie.enabled | bool | `true` | Set to false to disable the daily reminder email for overdue contact requests (per medewerker and per afdeling/groep) |
| verlopenContactverzoekHerinneringNotificatie.schedule | string | `"0 7 * * 1-5"` | Cron expression controlling when the daily reminder email is sent |
...
| web.styling.designTokensUrl | string | `""` |  |
| web.styling.faviconUrl | string | `""` |  |
| web.styling.logoUrl | string | `""` |  |
| web.styling.webFontSources | list | `[]` |  |
